...
import { CACHE_TAG_GAME_COUNT, CACHE_TAG_PLAYER_NAMES, CACHE_TAG_LOST_FLEET_COUNT } from '@/lib/cache';
...
export async function GET() {
  ...
  revalidateTag(CACHE_TAG_LOST_FLEET_COUNT, { expire: 0 });
  // Also drop the home page's cached HTML so the fresh count is rendered.
  revalidatePath('/');

  return NextResponse.json({
    revalidated: [CACHE_TAG_GAME_COUNT, CACHE_TAG_PLAYER_NAMES, CACHE_TAG_LOST_FLEET_COUNT, '/'],
    now: Date.now(),
  });
}
